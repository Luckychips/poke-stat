...
export default function Content() {
    ...
    const doFetch = useCallback(async () => {
        ...
        const r = await fetch(`https://pokeapi.co/api/v2/item/?limit=${limit}&offset=${offset}`)
        if (r.ok) {
            ...
            if (d.results.length) {
                ...
                setList(d.results.map((item: any) => {
                    ...
                    return {
                        id: id,
                        name: item.name,
                        thumbnailUrl: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/${item.name}.png`,
                        url: item.url,
                    }
                }));
            }
                            ...
                            {list.map((item, index) => {
                                return (
                                    <li key={`poke-dex-list-${item.id}`} className="flex flex-row items-center p-1">
                                        ...
                                        {item.thumbnailUrl && <ImageLoader src={item.thumbnailUrl} alt={item.name} width={30} height={30} />}
                                        <span
